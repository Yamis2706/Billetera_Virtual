...
public class TransaccionController {
    ...
    private static final String[] CATEGORIAS = {"Comida", "Transporte", "Entretenimiento", "Salud", "Educación"};
    ...
    private void mostrarTransaccion(Transaccion t) {
        if (t != null) {
            ...
            cbCategoria.setValue(t.getCategoria() != null ? t.getCategoria().getNombre() : null);
        }
    }
    ...
    private void crearTransaccion() {
        try {
            ...
            // Generar un id único corto para la transacción (8 caracteres)
            String idTransaccion = UUID.randomUUID().toString().substring(0, 5);

            // Buscar la categoría por nombre
            Categoria categoria = DataUtil.cargarCategorias().stream()
    ...
    private void filtrarPorCategoria() {
        String filtro = cbFiltroCategoria.getValue();
        if (filtro != null) {
            List<Transaccion> filtradas = listaTransacciones.stream()
                    .filter(t -> t.getCategoria() != null && filtro.equals(t.getCategoria().getNombre()))
